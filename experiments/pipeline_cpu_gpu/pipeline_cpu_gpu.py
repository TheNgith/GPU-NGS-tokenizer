#!/usr/bin/env python3
"""
CPU→GPU Pipeline: tokenize sequences on CPU → run Embedding + BiGRU model on GPU.

Usage:
    python pipeline_cpu_gpu.py --input /path/to/sequences.txt --output /path/to/output.csv
...
def main():
    parser = argparse.ArgumentParser(description="CPU tokenization + GPU inference pipeline")
    ...
    print(f"[CPU→GPU Pipeline] Tokenizing {args.input} with {args.threads} thread(s)...")
    t0 = time.perf_counter()
    tokens, io_time_us = cpu_tokenizer.tokenize(args.input, args.threads)
    t1 = time.perf_counter()
    print(f"[CPU→GPU Pipeline] Tokenization (incl I/O): {(t1 - t0)*1e6:.0f} us | io_time={io_time_us:.0f} us | shape={tokens.shape} dtype={tokens.dtype} device={tokens.device}")

    # ── Stage 1b: Transfer tokens to GPU ─────────────────────────────────
    tokens = tokens.to(device)
    t1b = time.perf_counter()
    print(f"[CPU→GPU Pipeline] Token transfer to GPU: {(t1b - t1)*1e6:.0f} us")

    # ── Padding check ────────────────────────────────────────────────────
    num_seqs = tokens.size(0)
    seq_len = tokens.size(1)
    assert seq_len == MAX_TOKENS, f"Expected seq_len={MAX_TOKENS}, got {seq_len}"
    print(f"[CPU→GPU Pipeline] {num_seqs} sequences, padded to {seq_len} tokens")
    ...
    print(f"[CPU→GPU Pipeline] Model inference: {(t3 - t2)*1e6:.0f} us | output shape={output_tensor.shape}")
    print(f"[CPU→GPU Pipeline] Total time: {(t3 - t0)*1e6:.0f} us")
