#!/usr/bin/env python3
"""
Naive GPU Pipeline: tokenize sequences on GPU (naive CUDA kernel)
                    → run Embedding + BiGRU model on GPU.

Usage:
    python pipeline_naive_gpu.py --input /path/to/sequences.txt --output /path/to/output.csv
...
naive_tokenizer = load(
    name="naive_tokenizer",
    sources=[os.path.join(_CSRC_DIR, "naive_tokenizer.cu")],
...
def main():
    parser = argparse.ArgumentParser(description="Naive GPU tokenization + GPU inference pipeline")
    ...
    # ── Stage 1: Tokenize on GPU (Naive CUDA kernel) ─────────────────────
    print(f"[Naive GPU Pipeline] Tokenizing {args.input} on GPU (naive kernel)...")
    t0 = time.perf_counter()
    tokens, io_time_us = naive_tokenizer.tokenize_naive_cuda(args.input)
    t1 = time.perf_counter()
    print(f"[Naive GPU Pipeline] Tokenization (incl I/O): {(t1 - t0)*1e6:.0f} us | io_time={io_time_us:.0f} us | shape={tokens.shape} dtype={tokens.dtype} device={tokens.device}")
    ...
    print(f"[Naive GPU Pipeline] {num_seqs} sequences, padded to {seq_len} tokens")
    ...
    print(f"[Naive GPU Pipeline] Model inference: {(t3 - t2)*1e6:.0f} us | output shape={output_tensor.shape}")
    print(f"[Naive GPU Pipeline] Total time: {(t3 - t0)*1e6:.0f} us")
